...
class DB(metaclass=MetaSingleton):
    ...
    def __init__(self):
        ...
        try:
            ...
            self.con_err_prom_counter.labels(ID).inc()
            self.logger.exception("A error occurred while connecting to the database")
            self.con_pool = None

    @query_prom_summary.labels(ID).time()
    def execute_query(self, query, params):
        con = self.con_pool.getconn()
        self.con_number_prom_gauge.labels(ID).inc()
        con.autocommit = True
        cursor = con.cursor()
        try:
            ...
            self.query_err_prom_counter.labels(ID).inc()
            self.logger.exception("A error occurred while executing query without returned results")
        finally:
            self.con_pool.putconn(con)
            self.con_number_prom_gauge.labels(ID).dec()

    @query_with_result_prom_summary.labels(ID).time()
    def execute_query_with_result(self, query, params):
        ...
        try:
            ...
            self.query_with_result_err_prom_counter.labels(ID).inc()
