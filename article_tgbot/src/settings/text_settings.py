...
help_admin = f"""Загрузите изображение с текстом для формирования статьи или просто текст с тегом /text, после чего 
выберите теги для статьи (не менее одног) и нажмите 'опубликовать'. Используйте тег: "{TAG_FOR_ALL_STUDENTS}" для того, 
чтобы отправить статью всем пользователям. Новый текст и фото будут обновлять вашу статью если она не опубликована."""
...
unallowable = "Данная команда недопустима для администратора"
